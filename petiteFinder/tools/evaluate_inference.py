...
def mean_average_precision(
        ...
        pred_json_path: str,
        IOU_threshold,
        gt_grande_catID: int = 1,
        gt_petite_catID: int = 2,
        pred_grande_catID: int = 0,
        pred_petite_catID: int = 1,
):
    """
        Function for computing mean average precision (mAP) of Petite/Grande detection.

            gt_json_path: str
                path to ground truth COCO json file

            pred_json_path: str
                path to predicted COCO json file

            IOU_threshold: list of floats or float
                intersection over union thresholds for mAP calculation

            gt_grande_catID: int
                category ID in json for ground truth Grande annotations

            gt_petite_catID: int
                category ID in json for ground truth Petite annotations

            pred_grande_catID: int
                category ID in json for predicted Grande annotations

            pred_petite_catID: int
                category ID in json for predicted Petite annotations
